...
def normalize_flatfile_channel(value: Any) -> str:
    ...
    # Dataset Selection can expose HN*; Flatfile accepts only HN/HG/HL/EH/HH.
    candidate = raw[:2]
...
def parse_event_time(value: Any) -> datetime:
    ...
    # ESM probe returns ISO-like UTC timestamps without an offset.
...
def build_flatfile_url(row: dict[str, Any], pad_seconds: int = 60) -> str:
    ...
    query = {
        # Do not pass Dataset Selection event_id here: Flatfile documents eventid as ESM-ID.
        "starttime": start,
        "endtime": end,
