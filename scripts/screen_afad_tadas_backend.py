...
DEFAULT_LEDGER = Path("results/local/afad_tadas/station_summary_backend_screen.csv")
DEFAULT_UI_LEDGER = Path("results/local/afad_tadas/station_summary_screen.csv")
DEFAULT_CANDIDATE_DIR = Path("data/private/tadas-backend-candidates")
FINAL_STATUSES = {"REJECT_SUMMARY_PGA", "CANDIDATE_COMPONENT_AUDIT"}
LEDGER_COLUMNS = (
    "rank","event_hash","event_id","event_date_from_export","backend_response_sha256",
    "backend_row_count","unique_station_count","stations_at_or_above_threshold",
    "max_summary_pga_cm_s2","threshold_cm_s2","required_candidate_stations","status",
    "reason","source_reference","candidate_json_path","screened_at_utc",
)
...
def build_payload_from_live_template(template:dict[str,object], row:dict[str,str], *, pad_days:int, shift:timedelta)->dict[str,object]:
    ...
    payload=copy.deepcopy(template); payload["eaEventId"]=event_id; payload["startDate"]=_serialize(start,shift); payload["endDate"]=_serialize(end,shift)
    return payload
...
def _load(path:Path, *, finals_only=False):
    if not path.exists(): return {}
    with path.open("r",encoding="utf-8",newline="") as h:
        rows=list(csv.DictReader(h))
    return {(r.get("event_id") or "").strip():r for r in rows if (r.get("event_id") or "").strip() and (not finals_only or r.get("status") in FINAL_STATUSES)}
...
def main()->int:
    ...
    entries=_load(a.ledger); ui=_load(a.ui_parity_ledger,finals_only=True); candidates=sum(r.get("status")=="CANDIDATE_COMPONENT_AUDIT" for r in entries.values()); parity=0; errors=0
    with KendoTadasPlaywrightBrowser(a.profile_dir,Path("data/private/tadas-backend-bootstrap-downloads"),headless=False,timeout_ms=a.timeout_ms,selectors={}) as browser:
        ...
        print(f"Backend bootstrap established; Authorization present=yes; observed date serialization shift={shift}"); print("Sensitive header values remain in memory only.")
        for row in rows:
            rank=int(row["rank"])
    ...
    print(f"Ledger: {a.ledger}"); print(f"Component-audit candidates accumulated: {candidates}"); print(f"Existing UI/CSV rows parity-checked: {parity}"); return 0
